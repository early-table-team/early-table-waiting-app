...
class TakeoutWaitingActivity : AppCompatActivity() {

    private lateinit var phoneNumberEditText: EditText
    private lateinit var personCountText: TextView
    private var personCount = 3
    private var waitingType : String = "TO_GO"
        ...
        apiService.createWaitingOffline(waitingRequest, selectedStore.id).enqueue(object : Callback<WaitingNumberResponseDto> {
            override fun onResponse(call: Call<WaitingNumberResponseDto>, response: Response<WaitingNumberResponseDto>) {
                if (response.isSuccessful) {
                    Log.d("TakeoutWaitingActivity", "API response successful: ${response.code()}")
                    ...
                    if (waitingResponse != null) {
                        ...
                        // waitingId 로그 출력
                        Log.d("WaitingIdCheck", "Received waitingId: $waitingId")

                        // SharedPreferences에 저장
                        val sharedPreferences = getSharedPreferences("app_prefs", MODE_PRIVATE)
                        val editor = sharedPreferences.edit()
                        editor.putString("TO_GO_waiting_id", waitingId.toString())
                        editor.apply()

                        // 저장된 값 확인 (선택적)
                        val savedWaitingId = sharedPreferences.getString("TO_GO_waiting_id", null)
                    ...
                    Log.e("TakeoutWaitingActivity", "API response failed: ${response.code()} $errorMsg")
                    runOnUiThread {
                        Toast.makeText(this@TakeoutWaitingActivity, "예약 등록에 실패했습니다.", Toast.LENGTH_SHORT).show()
                    }
                }
            }

            override fun onFailure(call: Call<WaitingNumberResponseDto>, t: Throwable) {
                Log.e("TakeoutWaitingActivity", "API call failed: ${t.message}")
                Toast.makeText(this@TakeoutWaitingActivity, "네트워크 오류", Toast.LENGTH_SHORT).show()
            }
        })
    }
}
